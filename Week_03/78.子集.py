...
class Solution:
    def subsets(self, nums: List[int]) -> List[List[int]]:
        ...
        # ans=[[]]
        # for i in nums:
        #     ans.extend([[i]+sub for sub in ans])
        # return ans

        # 3、递归-每个元素，可以选或者不选
        # def helper(ans,nums,lis,index):
        #     if index==len(nums):
        #         ans.append(lis[:])#拷贝一份新的数组，不用共用一份
        #         return #不要漏掉return
        #     helper(ans,nums,lis,index+1)#不选
        #     lis.append(nums[index])
        #     helper(ans,nums,lis,index+1)# 选
        #     # 没有reverse的话，可以helper(ans,nums,lis.copy(),index+1)
        ...
        #         ans.append(lis)
        #         return 
        #     helper(ans,nums,lis[:],index+1)
        #     helper(ans,nums,lis[:]+[nums[index]],index+1)#拷贝一份新的给孩子，不用担心孩子乱改
